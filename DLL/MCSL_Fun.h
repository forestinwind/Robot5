...
MCS_API int    MCS_CALL MCS_GetDeviceID(DWORD *dwVersion, WORD wCardIndex = 0);
...
MCS_API int    MCS_CALL MCS_SetMirrorAxis(int xMapToCh, int yMapToCh, int zMapToCh, int uMapToCh, int vMapToCh, int wMapToCh, int aMapToCh, int bwMapToCh, int nGroupIndex = 0);
MCS_API int    MCS_CALL MCS_SetComposeAxis(WORD wAxesMask = AXIS_X | AXIS_Y | AXIS_Z, WORD wGroupIndex =0);
...
MCS_API int    MCS_CALL MCS_SetMacParamEx(SYS_MAC_PARAM_EX *pstMacParam, WORD wChannel, WORD wCardIndex = 0);
MCS_API int    MCS_CALL MCS_GetMacParamEx(SYS_MAC_PARAM_EX *pstMacParam, WORD wChannel, WORD wCardIndex = 0);
...
MCS_API int    MCS_CALL MCS_SetPulseConfig(SYS_PULSE_CONFIG *pstPulseConfig, WORD wChannel, WORD wCardIndex = 0);
MCS_API int    MCS_CALL MCS_GetPulseConfig(SYS_PULSE_CONFIG *pstPulseConfig, WORD wChannel, WORD wCardIndex = 0);
...
MCS_API int    MCS_CALL MCS_InitSimulation(double dfInterpolateTime, SYS_CARD_CONFIG *pstCardConfig, WORD wCardNo);
...
MCS_API DWORD  PMC_GLB_GetDeviceID(WORD wCardIndex);
MCS_API DWORD  PMC_GLB_GetVendorID(WORD wCardIndex);
MCS_API int PMC_SetKey(int);
MCS_API int PMC_TryKey(int);
//////////////////////////////////////////////////////////////////////////////
// Local I/O Control
...
// LED On/Off Control
MCS_API int    MCS_CALL MCS_SetLedLightOn(WORD Channel, WORD wCardIndex = 0);
MCS_API int    MCS_CALL MCS_SetLedLightOff(WORD Channel, WORD wCardIndex = 0);
MCS_API int    MCS_CALL MCS_GetLedLightStatus(WORD Channel, WORD *Status, WORD wCardIndex = 0);
...
MCS_API int    MCS_CALL MCS_SetLimitFilterClk(WORD Value, WORD wCardIndex = 0);
MCS_API int    MCS_CALL MCS_SetHomeFilterClk(WORD Value, WORD wCardIndex = 0);
MCS_API int    MCS_CALL MCS_SetGpioFilterClk(WORD Value, WORD wCardIndex = 0);
...
MCS_API int    MCS_CALL MCS_DefinePosEx(WORD wAxis, double dfPos, WORD wCompensateMode = 0, WORD wGroupIndex = 0);
...
MCS_API double MCS_CALL MCS_SetFinalSpeed(double dfFinalSpeed, WORD wGroupIndex = 0);
...
MCS_API int    MCS_CALL MCS_GetSpeed(double *pdfVelX, double *pdfVelY, double *pdfVelZ, double *pdfVelU, double *pdfVelV, double *pdfVelW, double *pdfVelA, double *pdfVelB, WORD wGroupIndex = 0);
...
MCS_API int    MCS_CALL MCS_SetPtPSpeedEx(double dfRatioX, double dfRatioY, double dfRatioZ, double dfRatioU, double dfRatioV, double dfRatioW,  double dfRatioA, double dfRatioB, WORD wGroupIndex =0);
MCS_API int    MCS_CALL MCS_GetPtPSpeedEx(double *dfRatioX, double *dfRatioY, double *dfRatioZ, double *dfRatioU, double *dfRatioV, double *dfRatioW, double *dfRatioA, double *dfRatioB, WORD wGroupIndex =0);

// Point-to-Point Motion
MCS_API int    MCS_CALL MCS_PtP(double dfX, double dfY, double dfZ, double dfU, double dfV, double dfW, double dfA, double dfB, WORD wGroupIndex = 0, DWORD dwAxisMask = AXIS_ALL);
MCS_API int    MCS_CALL MCS_SyncPtP(double dfX, double dfY, double dfZ, double dfU, double dfV, double dfW, double dfA, double dfB, WORD wGroupIndex = 0, DWORD dwAxisMask = AXIS_ALL);
MCS_API int    MCS_CALL MCS_MirrorPtP(double dfX, double dfY, double dfZ, double dfU, double dfV, double dfW, double dfA, double dfB, WORD   wGroupIndex = 0, DWORD  dwAxisMask = AXIS_ALL);
...
// Linear Motion
MCS_API int    MCS_CALL MCS_Line(double dfX, double dfY, double dfZ, double dfU, double dfV, double dfW, double dfA, double dfB, WORD wGroupIndex = 0, DWORD dwAxisMask = AXIS_ALL);
...
MCS_API int    MCS_CALL MCS_SpiralXY_Z(double dfCX, double dfCY, double dfPitch, double dfCenterPitch, double dfTheta, WORD wGroupIndex = 0);
MCS_API int    MCS_CALL MCS_SpiralYZ_X(double dfCY, double dfCZ, double dfPitch, double dfCenterPitch, double dfTheta, WORD wGroupIndex = 0);
MCS_API int    MCS_CALL MCS_SpiralZX_Y(double dfCZ, double dfCX, double dfPitch, double dfCenterPitch, double dfTheta, WORD wGroupIndex = 0);
...
MCS_API int    MCS_CALL MCS_JogMirrorPulse(int nPulse, char cAxis, WORD wGroupIndex = 0);
...
MCS_API int    MCS_CALL MCS_SetHomeConfigEx(SYS_HOME_CONFIG_EX *pstHomeConfig, WORD wChannel, WORD wCardIndex = 0);
MCS_API int    MCS_CALL MCS_GetHomeConfigEx(SYS_HOME_CONFIG_EX *pstHomeConfig, WORD wChannel, WORD wCardIndex = 0);

// Operations
MCS_API int    MCS_CALL MCS_GoHome(int nXOrder, int nYOrder, int nZOrder, int nUOrder, int nVOrder, int nWOrder, int nAOrder, int nBOrder, WORD wTolerance, double dfEscapeTime, WORD wCardIndex = 0);
...
MCS_API int    MCS_CALL MCS_SetMaxCompPulse(WORD wMaxCompPulse, WORD wChannel, WORD wCardIndex = 0);
...
MCS_API int    MCS_CALL MCS_SetBlendMode(WORD wMode, WORD wGroupIndex = 0);
...
MCS_API double MCS_CALL MCS_OverridePtPSpeedEx(double dfRate, BOOL bInstant = 1, WORD wGroupIndex = 0);
MCS_API double MCS_CALL MCS_OverridePtPSpeed(double dfRate,  WORD wGroupIndex = 0);
...
MCS_API int    MCS_CALL MCS_GetENCValueEx(long long *plValue, WORD wChannel, WORD wCardIndex = 0);
...
MCS_API int    MCS_CALL MCS_SetENCLimitValue(long lValue, WORD wChannel, WORD wCardIndex = 0);
MCS_API int    MCS_CALL MCS_EnableENCLimitValue(WORD wEnable, WORD wChannel, WORD wCardIndex = 0);
...
MCS_API int	   MCS_CALL MCS_GetENCMirrorOffset(long *plLatchValue, WORD wChannel, WORD wCardIndex = 0);
...
MCS_API int    MCS_CALL MCS_Set3DCompareTolerance(WORD wTolerance, WORD wCardIndex = 0);
MCS_API int    MCS_CALL MCS_Enable3DCompareChannel(WORD wCompareAxisMask, WORD wCardIndex = 0);
...
MCS_API int    MCS_CALL MCS_EnableARIOSetControl(WORD wSet, WORD wCardIndex = 0);
MCS_API int    MCS_CALL MCS_DisableARIOSetControl(WORD wSet, WORD wCardIndex = 0);
...
MCS_API int    MCS_CALL MCS_SetGPIOTrigOutValueEx(DWORD dwBitMask, DWORD dwBitValue, WORD wCardIndex = 0);

MCS_API int    MCS_CALL MCS_GetGPIOCompStockCount(WORD *wCount, WORD wCardIndex = 0);
MCS_API int    MCS_CALL MCS_EraseGPIOCompValue(WORD wCardIndex = 0);
...
MCS_API int    MCS_CALL MCS_QueueDACOutputValue(WORD wChannel, float fVoltage, double dfDelayTime = 0, WORD wGroupIndex = 0);
...
//MCS_API int    MCS_CALL MCS_SetPWMRoutine(PWMISR pfnPWMRoutine, WORD wCardIndex = 0);
MCS_API int    MCS_CALL MCS_SetPWMFrequency(double dfFreq, WORD wCardIndex = 0); // dfFreq range from 0 ~ 5K Hz
MCS_API int    MCS_CALL MCS_SetPWMDuty(double dfDuty, WORD wChannel, WORD wCardIndex = 0); // dfDuty Range from 0 ~ 100
MCS_API int    MCS_CALL MCS_EnablePWMChannel(WORD wChannel, WORD wCardIndex = 0);
MCS_API int    MCS_CALL MCS_DisablePWMChannel(WORD wChannel, WORD wCardIndex = 0);
MCS_API int    MCS_CALL MCS_StartPWM(WORD wCardIndex = 0);
MCS_API int    MCS_CALL MCS_StopPWM(WORD wCardIndex = 0);
MCS_API int    MCS_CALL MCS_SetPWMRepeat(WORD wTimes, WORD wCardIndex = 0);
...
MCS_API double MCS_CALL MCS_SetDryRunFeedSpeed(double dfFeedSpeed, WORD wGroupIndex =0);
MCS_API double MCS_CALL MCS_GetDryRunFeedSpeed(WORD wGroupIndex =0);
...
MCS_API int    MCS_CALL MCS_SetOffsetTransform(	OFFSET_TRANSFORM pfnOffsetTransform, WORD wGroupIndex = 0 );
MCS_API int    MCS_CALL MCS_EnableOffsetTransform( WORD wGroupIndex = 0 );
MCS_API int    MCS_CALL MCS_DisableOffsetTransform( WORD wGroupIndex = 0 );

MCS_API int		ECM_LoadConfig(TCHAR *FilePath);
//MCS_API int		ECM_INIT(int nDoPdoConfig, uint16_t DCActCode, uint32_t CycTime);
MCS_API int		ECM_NewPdoConfTbl(int nSlvCnt);

MCS_API int		ECM_DelPdoConfTbl();
MCS_API int		ECM_SetPdoConfEnable(int nEnable);
MCS_API int		ECM_GetPdoConfEnable();
MCS_API int		ECM_SetPdoAsDrive(int nSlvIdx, int nType);

MCS_API void    MCS_CALL MCS_JogPulseEnter(void);
MCS_API void    MCS_CALL MCS_JogPulseExit(void);
MCS_API void    MCS_CALL MCS_SetMaxPulseStockCnt(int nMaxStockCnt);

MCS_API int    MCS_CALL ECM_EcatSdoWrite(uint8_t Slave,  uint16_t Index,  uint8_t SubIndex,  uint16_t size,  int Timeout,  uint16_t *Data);
MCS_API int    MCS_CALL ECM_EcatSdoRead(uint8_t Slave, uint16_t Index, uint8_t SubIndex, uint16_t size, int Timeout, uint8_t *Data);
MCS_API int    MCS_CALL ECM_GetObjAddr(int nLV, int nCh, int nType, int32_t **pAddr);
                        //nLV固定給0
                        //nType : 0   =  0x6041 状态字
                        //nType : 1   =  0x6064 编码器反馈位置(绝对位置)
                        //nType : 2   =  0x6040 控制字
                        //nType : 3   =  0x607A 目标位置指令
MCS_API void    _cdecl PMC_ENC_ReadCounter(WORD Channel, long* lCounter, WORD wCardIndex = 0);
MCS_API DWORD	_cdecl PMC_GetDrvVersion(); 
void	 PMC_GetVersion(char* Ver, WORD wCardIndex = 0);

MCS_API void	ECM_GetRxPdoDataBuf(void **pBuf);
MCS_API void	ECM_GetTxPdoDataBuf(void **pBuf);
MCS_API void	ECM_CopyToRxPdo(int nOffset, int nSize, uint8_t *pData);
MCS_API void	ECM_CopyFromTxPdo(int nOffset, int nSize, uint8_t *pData);

MCS_API void  PMC_EXT_MpgOpen(int nCh, WORD wCardIndex);
MCS_API void  PMC_EXT_MpgClose(int nCh, WORD wCardIndex);
MCS_API int	   ECM_SetPdoDefault(int nCfgIdx, int nSlvIdx, int nType);
MCS_API	int	 ECM_Set402CallBackFunc(ECM_402FUNC pFunc);	
MCS_API void ECM_ClearErrorFlag();
MCS_API int	ECM_GetErrorFlag();
typedef int(*ECM_402FUNC)(WORD wChannel, WORD wStatus, WORD *pwCtrl);
MCS_API void PMC_EXT_SetClr402Err(int nCh, WORD wCardIndex);
MCS_API uint32_t PMC_EXT_GetClr402Err(int nCh, WORD wCardIndex);
//MCS_API int	 ECM_Set402CallBackFunc(ECM_402FUNC pFunc);
//PMC_LIB_API void PMC_LIB_CALL PMC_EXT_SetClr402Err(int nCh, WORD wCardIndex);
	//PMC_LIB_API uint32_t PMC_LIB_CALL PMC_EXT_GetClr402Err(int nCh, WORD wCardIndex);
